...
# SE: share encoders
EXPERIMENT_NAME=${SUB_DATA_NAME}_asr_mt_st_DEPI_SE
...
if [ "${SRC_LANG}" = "en" ]; then
  ...
  ST_DATA_DIR=data/CoVoST2/preprocessed/${ST_SUB_DATA_NAME}/en-X
else
  DATA_DIR=data/CoVoST2/preprocessed/${SUB_DATA_NAME}/${SRC_LANG}-${TGT_LANG}
  ST_DATA_DIR=data/CoVoST2/preprocessed/${ST_SUB_DATA_NAME}/${SRC_LANG}-${TGT_LANG}
fi
CONCAT=4
...
if [ -d ${ST_DATA_DIR}/${SUB_DIR} ]; then
  ...
  echo "Preprocessing ${SUB_DIR} data"
  mkdir ${ST_DATA_DIR}/${SUB_DIR}
  # Use the same vocabs as above while preprocessing
  # Preprocess ST data
  python preprocess.py -train_src $ST_DATA_DIR/${SRC_LANG}_audio_train.scp  \
      -train_tgt $ST_DATA_DIR/${TGT_LANG}_text_train.txt  \
      -valid_src $ST_DATA_DIR/${SRC_LANG}_audio_val.scp  \
      -valid_tgt $ST_DATA_DIR/${TGT_LANG}_text_val.txt  \
      -train_src_lang ${SRC_LANG} \
      -train_tgt_lang ${TGT_LANG} \
      -valid_src_lang ${SRC_LANG} \
      -valid_tgt_lang ${TGT_LANG} \
...
DATA_FORMAT=scp
ADDITIONAL_DATA="${DATA_DIR}/${SUB_DIR}/mt_data;${ST_DATA_DIR}/${SUB_DIR}/st_data"
