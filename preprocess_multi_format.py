...
parser.add_argument('-train_src_text', required=True,
                    help="Path to the text training source data")
parser.add_argument('-train_src_audio', required=True,
                    help="Path to the audio training source data")
parser.add_argument('-train_tgt_of_text', required=True,
                    help="Path to the training target data of the text source")
parser.add_argument('-train_tgt_of_audio', required=True,
                    help="Path to the training target data of the audio source")

parser.add_argument('-valid_src_text', required=True,
                    help="Path to the text validation source data")
parser.add_argument('-valid_src_audio', required=True,
                    help="Path to the audio validation source data")
parser.add_argument('-valid_tgt_of_text', required=True,
                    help="Path to the validation target data of the text source")
parser.add_argument('-valid_tgt_of_audio', required=True,
                    help="Path to the validation target data of the audio source")

parser.add_argument('-train_src_text_lang', default="src",
                    help="Language(s) of the text source sequences")
parser.add_argument('-train_tgt_of_text_lang', default="tgt",
                    help="Language(s) of the target sequences of the text source.")
parser.add_argument('-train_src_audio_lang', default="src",
                    help="Language(s) of the audio source sequences")
parser.add_argument('-train_tgt_of_audio_lang', default="tgt",
                    help="Language(s) of the target sequences of the audio source.")

parser.add_argument('-valid_src_text_lang', default="src",
                    help="Language(s) of the text source sequences.")
parser.add_argument('-valid_tgt_of_text_lang', default="tgt",
                    help="Language(s) of the target sequences of the text source.")
parser.add_argument('-valid_src_audio_lang', default="src",
                    help="Language(s) of the audio source sequences.")
parser.add_argument('-valid_tgt_of_audio_lang', default="tgt",
                    help="Language(s) of the target sequences of the audio source.")
...
parser.add_argument('-src_text_seq_length', type=int, default=10000,
                    help="Maximum source text sequence length")
parser.add_argument('-src_audio_seq_length', type=int, default=10000,
                    help="Maximum source audio sequence length")
parser.add_argument('-src_seq_length_trunc', type=int, default=0,
                    help="Truncate source sequence length.")
parser.add_argument('-tgt_of_text_seq_length', type=int, default=10000,
                    help="Maximum target sequence length to keep.")
parser.add_argument('-tgt_of_audio_seq_length', type=int, default=10000,
                    help="Maximum target sequence length to keep.")
...
def main():
    ...
    src_text_langs = opt.train_src_text_lang.split("|")
    src_audio_langs = opt.train_src_audio_lang.split("|")
    tgt_of_text_langs = opt.train_tgt_of_text_lang.split("|")
    tgt_of_audio_langs = opt.train_tgt_of_audio_lang.split("|")
    langs = (src_text_langs + src_audio_langs + tgt_of_text_langs + tgt_of_audio_langs)
    ...
    src_train_text_files = opt.train_src_text.split("|")
    src_train_audio_files = opt.train_src_audio.split("|")
    tgt_of_text_train_files = opt.train_tgt_of_text.split("|")
    tgt_of_audio_train_files = opt.train_tgt_of_audio.split("|")

    # Build vocab for text data
    if opt.join_vocab:
        dicts['src'] = init_vocab('source', set(src_train_text_files + tgt_of_text_train_files +
                                                tgt_of_audio_train_files), opt.src_vocab,
                                  opt.tgt_vocab_size, tokenizer, num_workers=opt.num_threads)
        dicts['tgt'] = dicts['src']
    else:
        dicts['src'] = init_vocab('source', src_train_text_files, opt.src_vocab,
                                  opt.src_vocab_size, tokenizer, num_workers=opt.num_threads)

        dicts['tgt'] = init_vocab('target', set(tgt_of_text_train_files + tgt_of_audio_train_files), opt.tgt_vocab,
                                  opt.tgt_vocab_size, tokenizer, num_workers=opt.num_threads)
    ...
    # Prepare data
    print('Preparing training mixed model ...')
    start = time.time()
    train = prepare_split(dicts, tokenizer, opt.train_src_text, opt.train_tgt_of_text, opt.train_src_text_lang,
                          opt.train_tgt_of_text_lang, opt.train_src_audio, opt.train_tgt_of_audio, opt.train_src_audio_lang,
                          opt.train_tgt_of_audio_lang)

    print('Preparing validation ...')
    valid = prepare_split(dicts, tokenizer, opt.valid_src_text, opt.valid_tgt_of_text, opt.valid_src_text_lang,
                          opt.valid_tgt_of_text_lang, opt.valid_src_audio, opt.valid_tgt_of_audio, opt.valid_src_audio_lang,
                          opt.valid_tgt_of_audio_lang)
    elapse = str(datetime.timedelta(seconds=int(time.time() - start)))
    print("Prepare mixed data finished after %s" % elapse)

    if opt.src_vocab is None and opt.asr == False and opt.lm == False:
        save_vocabulary('source', dicts['src'], opt.save_data + '.src.dict')
    if opt.src_vocab is None and opt.src_type == 'mix':
        save_vocabulary('source', dicts['src'], opt.save_data + '.src.dict')
...
def prepare_split(dicts, tokenizer, src_text, tgt_of_text, src_text_lang, tgt_of_text_lang, src_audio, tgt_of_audio, src_audio_lang,
                  tgt_of_audio_lang):
    split_dict = dict()
    split_dict['src'], split_dict['tgt'] = list(), list()
    split_dict['src_sizes'], split_dict['tgt_sizes'] = list(), list()
    split_dict['src_lang'], split_dict['tgt_lang'] = list(), list()

    # Add text samples
    split_dict = add_samples(dicts, tokenizer, split_dict=split_dict, sample_format='text', src=src_text, tgt=tgt_of_text,
                             src_lang=src_text_lang, tgt_lang=tgt_of_text_lang)
    # Add audio samples
    split_dict = add_samples(dicts, tokenizer, split_dict=split_dict, sample_format='audio', src=src_audio, tgt=tgt_of_audio,
                             src_lang=src_audio_lang, tgt_lang=tgt_of_audio_lang)

    return split_dict


def add_samples(dicts, tokenizer, split_dict, sample_format, src, tgt, src_lang, tgt_lang):
    src_input_files = src.split("|")
    tgt_input_files = tgt.split("|")
    src_langs = src_lang.split("|")
    tgt_langs = tgt_lang.split("|")
    ...
    for (src_file, tgt_file, src_lang, tgt_lang) in zip(src_input_files, tgt_input_files,
                                                        src_langs, tgt_langs):
        if sample_format == "text":
            src_data, tgt_data, src_sizes, tgt_sizes = make_translation_data(src_file, tgt_file,
                                                                             dicts['src'], dicts['tgt'], tokenizer,
                                                                             max_src_length=opt.src_text_seq_length,
                                                                             max_tgt_length=opt.tgt_of_text_seq_length,
                                                                             add_bos=(not opt.no_bos),
                                                                             data_type=opt.data_type,
                                                                             num_workers=opt.num_threads,
                                                                             verbose=opt.verbose)
        elif sample_format == "audio":
            src_data, tgt_data, src_sizes, tgt_sizes = make_asr_data(src_file, tgt_file,
                                                                     dicts['tgt'], tokenizer,
                                                                     max_src_length=opt.src_audio_seq_length,
                                                                     max_tgt_length=opt.tgt_of_audio_seq_length,
            ...
            raise RuntimeError("Format not known.")
        ...
        split_dict['src'] += src_data
        split_dict['tgt'] += tgt_data
        split_dict['src_sizes'] += src_sizes
        split_dict['tgt_sizes'] += tgt_sizes
        split_dict['src_lang'] += src_lang_data
        split_dict['tgt_lang'] += tgt_lang_data
    return split_dict
