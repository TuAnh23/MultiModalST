...
if [ "${TGT_LANG}" = "en" ]; then
  OP_DATA_DIR=data/CoVoST2/preprocessed/${OP_SUB_DATA_NAME}/en-X
else
  OP_DATA_DIR=data/CoVoST2/preprocessed/${OP_SUB_DATA_NAME}/${TGT_LANG}-${SRC_LANG}
fi
CONCAT=4
SUB_DIR=${SRC_MODALITY}_${SRC_LANG}_${TGT_MODALITY}_${TGT_LANG}_bi
SRC_TGT_VOCAB=$DATA_DIR/${SUB_DIR}/src_tgt_vocab
echo "Preprocess main direction data"
if [ -d ${DATA_DIR}/${SUB_DIR} ]; then
  ...
  python vocab_generator.py -filenames "$DATA_DIR/${SRC_LANG}_text_train.txt|${DATA_DIR}/${TGT_LANG}_text_train.txt|$OP_DATA_DIR/${SRC_LANG}_text_train.txt|${OP_DATA_DIR}/${TGT_LANG}_text_train.txt" \
...
echo "Preprocess opposite direction data"
if [ -d ${OP_DATA_DIR}/${SUB_DIR} ]; then
  echo "${SUB_DIR} already preprocessed"
else
  echo "Preprocessing ${SUB_DIR} data"
  mkdir ${OP_DATA_DIR}/${SUB_DIR}
  # Use the above vocabs while preprocessing
  # Preprocess ASR data
  python preprocess.py -train_src $OP_DATA_DIR/${TGT_LANG}_audio_train.scp  \
      -train_tgt $OP_DATA_DIR/${TGT_LANG}_text_train.txt  \
      -valid_src $OP_DATA_DIR/${TGT_LANG}_audio_val.scp  \
      -valid_tgt $OP_DATA_DIR/${TGT_LANG}_text_val.txt  \
      -train_src_lang ${TGT_LANG} \
      -train_tgt_lang ${TGT_LANG} \
      -valid_src_lang ${TGT_LANG} \
      -valid_tgt_lang ${TGT_LANG} \
      -all_langs "${SRC_LANG}|${TGT_LANG}" \
      -src_seq_length 1024  \
      -tgt_seq_length 512  \
      ...
      -src_type audio \
      -asr_format scp \
      -save_data $OP_DATA_DIR/${SUB_DIR}/asr_data \
      -format scp \
      -tgt_vocab $SRC_TGT_VOCAB
  # Preprocess MT data
  python preprocess.py -train_src $OP_DATA_DIR/${TGT_LANG}_text_train.txt  \
      -train_tgt $OP_DATA_DIR/${SRC_LANG}_text_train.txt  \
      -valid_src $OP_DATA_DIR/${TGT_LANG}_text_val.txt  \
      -valid_tgt $OP_DATA_DIR/${SRC_LANG}_text_val.txt  \
      -train_src_lang ${TGT_LANG} \
      -train_tgt_lang ${SRC_LANG} \
      -valid_src_lang ${TGT_LANG} \
      -valid_tgt_lang ${SRC_LANG} \
      ...
      -concat 1 \
      -src_type text \
      -save_data $OP_DATA_DIR/${SUB_DIR}/mt_data \
      -format mmem \
      -src_vocab $SRC_TGT_VOCAB \
      -tgt_vocab $SRC_TGT_VOCAB
fi
...
DATA_FORMAT=scp
ADDITIONAL_DATA="${DATA_DIR}/${SUB_DIR}/mt_data;${OP_DATA_DIR}/${SUB_DIR}/asr_data;${OP_DATA_DIR}/${SUB_DIR}/mt_data"
